...
import me.ministrie.gui.types.holders.skin.SkinCrossbowWeaponGuiHolder;
...
public class SkinCrossbowWeaponGui implements Screen, Pageable, ButtonAction<BowSkin>{
	...
	public SkinCrossbowWeaponGui(MamanghoPlayer viewer){
		this.viewer = viewer;
		this.holder = new SkinCrossbowWeaponGuiHolder(this);
	}
	...
	protected Inventory getPanel(){
		...
		Inventory panel = Bukkit.createInventory(holder, 54, ComponentUtil.parseComponent("쇠뇌 스킨 설정"));
	...
	public void refresh(){
		...
		if(holder != null && holder instanceof SkinCrossbowWeaponGuiHolder){
	...
	public void click(InventoryClickEvent event){
		...
		if((skin = this.getAction(slot)) != null){
			if(type.equals(ClickType.NUMBER_KEY)){
				...
				if(hotkey == 0){
					BowSkin current = this.viewer.getData().<BowSkin>getData(DataEnum.CROSSBOW_SKIN);
					boolean same = current != null && current.getUID().equals(skin.getUID());
					if(same){
						if(this.viewer.getData().setData(DataEnum.CROSSBOW_SKIN, null)){
						...
						if(this.viewer.getData().setData(DataEnum.CROSSBOW_SKIN, skin)){
							SoundSetting.DEFAULT_GUI_PAGE_CLICK.playSound(viewer);
							ProtocolLibrary.getProtocolManager().updateEntity(this.viewer.getPlayer(), this.viewer.getPlayer().getWorld().getPlayers());
							this.refresh();
							this.viewer.getPlayer().updateInventory();
			...
			if(type.equals(ClickType.NUMBER_KEY)){
				...
				if(hotkey == 0){
					BowSkin current = this.viewer.getData().<BowSkin>getData(DataEnum.CROSSBOW_SKIN);
					if(current != null){
						this.viewer.getData().setData(DataEnum.CROSSBOW_SKIN, null);
						SoundSetting.DEFAULT_GUI_PAGE_CLICK.playSound(viewer);
						ProtocolLibrary.getProtocolManager().updateEntity(this.viewer.getPlayer(), this.viewer.getPlayer().getWorld().getPlayers());
						this.refresh();
						this.viewer.getPlayer().updateInventory();
	...
	protected void postProcess(Inventory panel){
		...
		BowSkin skin = data.getData(DataEnum.CROSSBOW_SKIN);
		String crossbow = skin != null ? skin.getName() : MessageSetting.FORMAT_SKIN_EMPTY.getValue();
		panel.setItem(UNSET_SLOT, IconSetting.SKIN_CROSSBOW_UNSET_ICON.getIconInfo().toIcon(crossbow));
	}
}
