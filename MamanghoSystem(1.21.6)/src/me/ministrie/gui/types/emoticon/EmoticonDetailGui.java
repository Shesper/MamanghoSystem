...
public class EmoticonDetailGui extends EmoticonScreen implements Screen, Pageable, ButtonAction<Emoticon>{
	...
	private EmoticonPackage emoticon;
	private Emoticon[] actions;
	
	private int page;
	private Map<Integer, List<Emoticon>> pages = new HashMap<>();
	
	public EmoticonDetailGui(MamanghoPlayer viewer, EmoticonPackage emoticon){
		this.viewer = viewer;
		this.emoticon = emoticon;
		this.holder = new EmoticonDetailGuiHolder(this);
	}
	...
	public void initializePages(){
		...
		int page = 0;
		int offset = 0;
		List<Emoticon> list = new ArrayList<>(this.emoticon.getEmojis());
	...
	public void refresh(){
		...
		if(holder != null && holder instanceof EmoticonDetailGuiHolder){
	...
	protected Inventory getPanel(){
		...
		Inventory panel = Bukkit.createInventory(holder, 54, ComponentUtil.parseComponent(StringUtils.stripColor(this.emoticon.getName()) + " 디시콘 리스트"));
		...
		if(page != null && !page.isEmpty()){
			...
			for(Emoticon e : page){
				panel.setItem(slot, e.getUIIcon(bookmark.isBookmarked(e), this.getHookPairFirst()));
				actions[slot] = e;
				slot++;
			}
		}
	...
	public void click(InventoryClickEvent event){
		...
		if((emoticon = getAction(slot)) != null){
			...
			if(click.equals(ClickType.LEFT)){
				...
				EmoticonBookmark bookmark = this.viewer.getData().getData(DataEnum.EMOTICON_BOOKMARK, EmoticonBookmark.emptyBookmark());
				bookmark.bookmark(emoticon, !bookmark.isBookmarked(emoticon), this.viewer.getData());
